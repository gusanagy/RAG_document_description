...
"""
app_chat_runner.py
Streamlit que executa seu query_faiss_index.py ao perguntar e preenche
automaticamente as respostas (structured e llm), além de exibir as fontes (hits).

Como usar:
    streamlit run app_chat_runner.py
...
def run_cli(cmd: List[str], env: Optional[Dict[str, str]] = None, timeout: Optional[int] = None) -> subprocess.CompletedProcess:
    """Roda um comando e retorna o CompletedProcess com stdout/stderr capturados."""
...
def extract_answer_from_stdout(stdout: str) -> str:
    """
    Fallback: tenta extrair a resposta do stdout do script quando não foi salvo via arquivo.
    Procura pelo marcador '[2/2] Resposta' e pega o bloco até o fim.
    """
    # tenta encontrar a linha com "[2/2] Resposta" e capturar tudo após ela
    m = re.search(r"^\[2/2\].*?(?:Respost[ao].*?:)?\s*\n(.*)$", stdout, flags=re.S | re.I | re.M)
    if m:
        return m.group(1).strip()
    # plano B: se o script imprime "--- RESPOSTA ---"
    m = re.search(r"---\s*RESPOSTA\s*---\s*\n(.*)$", stdout, flags=re.S | re.I)
    if m:
        return m.group(1).strip()
    # fallback final: devolve stdout completo (não ideal, mas útil para depurar)
    return stdout.strip()


def load_hits_if_exist(path: Optional[Path]) -> List[Dict[str, Any]]:
...
def build_cli_args(
    ...
    answer_out_path: Optional[Path] = None,
) -> List[str]:
    """
    Monta a linha de comando para rodar o query_faiss_index.py com os parâmetros fornecidos.
    Se export_hits_path for dado, adiciona --export-hits (se seu script tiver).
    Não vamos exigir --answer-out (não estava no seu script); vamos capturar do stdout.
    """
    cmd = [
    ...
    # Se seu script tiver essa flag adicionada (opcional que sugerimos), use:
...
def run_both_modes(
    ...
    """
    Executa o script duas vezes e retorna dict com:
      {
        "structured_text": ...,
        "llm_text": ...,
        "hits": [...]     # se --export-hits existir no script; senão, pode vir vazio
        "stdout_structured": ...,
        "stdout_llm": ...,
        "stderr_structured": ...,
        "stderr_llm": ...
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        hits_json = tmp / "hits.json"  # tentaremos exportar os hits aqui
        ...
        cmd_struct = build_cli_args(
            ...
            export_hits_path=hits_json
        )
        cp_s = run_cli(cmd_struct)
        result["stdout_structured"] = cp_s.stdout
        result["stderr_structured"] = cp_s.stderr
        structured_answer = extract_answer_from_stdout(cp_s.stdout)
        result["structured_text"] = structured_answer

        # Tenta ler hits exportados (se o script não tiver --export-hits, este arquivo não existirá)
        hits = load_hits_if_exist(hits_json)
        result["hits"] = hits
        ...
        cmd_llm = build_cli_args(
            ...
            gen_model=gen_model, limit_context_hits=limit_context_hits, max_context_chars=max_context_chars,
            max_new_tokens=max_new_tokens, dtype=dtype, bits=bits, gpu_mem=gpu_mem,
            export_hits_path=hits_json  # tentar reexportar (ou sobrescrever) não tem problema
        )
        cp_l = run_cli(cmd_llm)
        result["stdout_llm"] = cp_l.stdout
        result["stderr_llm"] = cp_l.stderr
        llm_answer = extract_answer_from_stdout(cp_l.stdout)
        result["llm_text"] = llm_answer

        # Se não leu hits antes, tenta novamente
        if not result["hits"]:
            result["hits"] = load_hits_if_exist(hits_json)
...
def main():
    ...
    with st.sidebar:
        ...
        script_path = st.text_input(
            ...
            value=str(Path.cwd() / "src/query_faiss_index.py")
        )
        ...
        st.markdown("---")
        auto_export = st.checkbox("Tentar usar --export-hits (se suportado)", value=True)
    ...
    if query:
        ...
        if structured_text:
            ...
            st.warning("Sem saída structured detectada. Veja 'stdout (structured)' abaixo.")

        if llm_text:
            color_badge("llm")
            bubble(llm_text, sender="assistant")
        else:
            st.warning("Sem saída llm detectada. Veja 'stdout (llm)' abaixo.")
        ...
        with st.expander("Fontes (hits)"):
            if hits:
                ...
                st.info("Sem hits exportados. Adicione a flag --export-hits no seu script para habilitar.")
